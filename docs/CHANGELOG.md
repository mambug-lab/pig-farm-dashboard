...
2026-09-26 이후 추가하는 기록의 시각은 Asia/Vladivostok (UTC+10)이다. 아래 과거 커밋·파일 시각은 별도 표기가 없으면 당시 기록의 UTC를 유지한다. 기존 대화 요약의 날짜는 해당 요약 표기를 유지한다. 날짜를 알 수 없는 수정은 임의로 날짜를 부여하지 않는다. 여기서 ‘확인’은 아래 구분에 한정되며 운영 성공으로 일괄 해석하지 않는다.
...
## 2026-09-26 — Plugin 분리본 운영 적용 및 HTML 메타 조회 수정

**운영 Apps Script 반영 / GitHub 소스 정합화 / HTML 공개용 변경**

- 운영에 25개 GS + appsscript.json을 반영하고 16:05에 GET 전수 대조 26/26 일치 확인.
- 월간·연간 단독 갱신에 문서 잠금 적용, 진단 5개에는 상태 열을 쓰지 않는 분석 경로 연결.
- 기존 4개 트리거 유지. 파일 분리를 위한 트리거 재설치 없음.
- 저장소의 단일 Code.gs를 운영과 같은 분리 파일로 전환. appscript.json을 실제 파일명 appsscript.json으로 교정하고 Asia/Vladivostok으로 통일.
- index.html의 월간 메타 조회를 6개 범위로 나누고 원래 숫자값을 보존. L3 fallback 제거, 기존 날짜·공식 그룹 재고 계약 유지.
- 기존 HTML 로컬 11개 검사 및 한국어 실데이터 화면 확인 결과를 재사용. 같은 기능 검사를 다시 수행하지 않음.
- 운영 연동값 읽기에서 총 5,360, 모돈 373, MSY 9.65 등 주요 메타가 이전 확인값과 일치.
- 운영 수동 재계산 검증은 필수로 두지 않고 생략하기로 함. 재시도 완료 기록을 전체 재계산 증거로 확대하지 않음.
- 현황·시트 연결 규약·운영 README와 이번 릴리스 및 복구 설명 갱신.

이 커밋의 GitHub Pages 공개 결과는 해당 **pages build and deployment** 작업과 실제 공개 화면을 기준으로 확인한다. 정규 월초 자연 발화는 별도 관찰 항목이다.

상세: [PLUGIN_RELEASE_2026-09-26.md](PLUGIN_RELEASE_2026-09-26.md).

## 2026-09-23 — v11.5-RC10 적용 및 검증

**코드·HTML 수정 / 테스트 사본 실검증 / 운영 반영**

- 관리사무소 대장 기준 공식 그룹재고가 수기 월초 재고로 초기화되지 않고 월 경계를 이어가도록 계산을 수정.
- HTML에서 공식 그룹재고, 상세 돈방 합계, 미배분 보정잔차를 구분해 표시하고 공식 메타 계약을 검증하도록 수정.
- 로컬 회귀검사 75건 통과, 실패 0건.
- 독립 테스트 사본에서 월 경계 연속성과 편집·변경·재시도·월 생성 트리거의 실제 실행을 확인.
- 테스트 프로젝트의 검증용 트리거 5개를 모두 완전삭제하고 0개 상태를 확인.
- 운영 Apps Script에 RC10을 저장하고 대시보드 트리거 3개와 월 생성 트리거 1개를 설치.
- 운영 `refreshDashboardLinks()` 실행 완료. 월간 대상 `Август 2026`, 유효 작업일 26일, 최신 유효일 2026-08-26, 연간 출력 4,626행.

검증 보고서: [`RC10_LIVE_VALIDATION_2026-09-23.md`](RC10_LIVE_VALIDATION_2026-09-23.md).
